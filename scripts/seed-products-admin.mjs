/**
 * Seeds the Firestore `products` collection using the Firebase Admin SDK.
 * Run with: node scripts/seed-products-admin.mjs
 */
...
// Try to use a service account file if provided, otherwise try ADC
let appConfig = { projectId: envVars.VITE_FIREBASE_PROJECT_ID };
...
async function seed() {
  ...
  for (const product of products) {
    ...
    if (existingIds.has(id)) {
      console.log(`  SKIP  ${id}: ${product.name}`);
      skipped++;
      continue;
    }
